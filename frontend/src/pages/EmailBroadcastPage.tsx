...
export const EmailBroadcastPage: React.FC = () => {
  ...
  // Current Admin Email Credentials State
  const [currentAdminInfo, setCurrentAdminInfo] = useState<{ id: string; name: string; email: string; has_credentials: boolean } | null>(null);
  const [adminAppPassInput, setAdminAppPassInput] = useState<string>('');
  const [savingAdminPass, setSavingAdminPass] = useState<boolean>(false);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setCurrentAdminInfo(recipRes.data?.current_admin || null);
  ...
  // 1-Click Direct Send via Logged-in Admin Gmail Web Client
  const handleSendViaGmailClient = async () => {
    if (!subject.trim()) {
      showNotification('error', 'Please provide an Email Subject before opening Gmail.');
      return;
    }
    ...
    if (allSelectedEmails.length === 0) {
      showNotification('error', 'Please select at least one recipient email address.');
      return;
    }

    try {
      // Record dispatch in portal database and audit history
      const payload = {
      ...
      await api.post('/email-broadcast/record-direct-send', payload);
      fetchData();
    } catch (e) {
      console.warn('Could not record direct send history:', e);
    }

    // Construct Gmail Web Compose URL with user email, BCC recipients, Subject, and Body
    const bccParam = allSelectedEmails.join(',');
    const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&authuser=${encodeURIComponent(user?.email || '')}&bcc=${encodeURIComponent(bccParam)}&su=${encodeURIComponent(subject.trim())}&body=${encodeURIComponent(message.trim())}`;
    ...
    showNotification(
      'success',
      `Opening Gmail for ${user?.email}... All ${allSelectedEmails.length} recipients are loaded in BCC. Simply click 'Send' in Gmail!`
    );

    setShowPreviewModal(false);
  };

  // Save 16-character app password for the logged-in admin
  const handleSaveAdminPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!adminAppPassInput.trim()) {
      showNotification('error', 'Please enter your 16-character Google App Password.');
      return;
    }
    setSavingAdminPass(true);
    try {
      const res = await api.post('/email-broadcast/admin-email-credentials', {
        email_app_password: adminAppPassInput.trim()
      });
      showNotification('success', res.data?.message || 'Email password saved successfully for your account!');
      setAdminAppPassInput('');
      fetchData();
    } catch (err: any) {
      showNotification('error', err.response?.data?.message || 'Failed to save email password.');
    } finally {
      ...
      {activeMainTab === 'compose' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-7 space-y-5">
            ...
            {/* Logged-In Admin Direct Sender Identity */}
            <div className="p-4 bg-gradient-to-r from-purple-50 via-sky-50 to-indigo-50 border border-purple-200/80 rounded-2xl text-xs space-y-2 shadow-xs">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse" />
                  <span className="font-black text-purple-950 text-xs">Sending Directly From Your Logged-in Account:</span>
                </div>
                <span className="px-2.5 py-0.5 bg-purple-200 text-purple-900 font-extrabold rounded-full text-[10px] tracking-wide">
                  {user?.email === 'pranavannur9659@gmail.com' ? '👑 Super Admin' : 'Admin'}
                </span>
              </div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between text-[11px] text-slate-700 gap-2 pt-1 border-t border-purple-100">
                <div className="flex items-center space-x-1.5">
                  <Mail className="w-3.5 h-3.5 text-purple-600 shrink-0" />
                  <span className="font-extrabold text-slate-900">{user?.name}</span>
                  <span className="text-slate-400">•</span>
                  <span className="font-mono font-bold text-purple-700 bg-white px-2 py-0.5 rounded border border-purple-200 shadow-xs">
                    {user?.email}
                  </span>
                </div>
                <div className="text-[10px] text-slate-500 flex items-center space-x-1">
                  <span>All recipients see your email ID as the sender</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3">
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 text-xs font-extrabold rounded-xl border border-slate-200 flex items-center justify-center space-x-1.5 transition-all cursor-pointer"
              >
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
                {/* 1-Click Direct Gmail Dispatch */}
                <button
                  type="button"
                  onClick={handleSendViaGmailClient}
                  disabled={allSelectedEmails.length === 0 || !subject.trim() || !message.trim()}
                  className="w-full sm:w-auto px-5 py-2.5 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-xs font-black rounded-xl shadow-md flex items-center justify-center space-x-2 transition-all cursor-pointer"
                  title={`Directly opens your Gmail (${user?.email}) with all recipients pre-filled in BCC`}
                >
                  <Mail className="w-4 h-4" />
                  <span>Send via My Gmail ({allSelectedEmails.length})</span>
                </button>

                {/* In-Portal Automated Dispatch */}
                <button
                  type="button"
                  onClick={handleSendBroadcast}
                  disabled={sending || allSelectedEmails.length === 0 || !subject.trim() || !message.trim()}
                  className="w-full sm:w-auto px-5 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white text-xs font-black rounded-xl shadow-md flex items-center justify-center space-x-2 transition-all cursor-pointer"
                >
                  <Send className="w-4 h-4" />
                  <span>{sending ? 'Sending...' : `Dispatch in Portal (${allSelectedEmails.length})`}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl space-y-5 animate-in zoom-in-95">
            ...
            <div className="flex flex-col sm:flex-row justify-end items-center gap-2 pt-2">
              ...
              <button
                type="button"
                onClick={handleSendViaGmailClient}
                className="w-full sm:w-auto px-5 py-2 bg-red-600 hover:bg-red-700 text-white font-black text-xs rounded-xl shadow-md flex items-center justify-center space-x-1.5 transition-all cursor-pointer"
                title={`Open your Gmail (${user?.email}) with ${allSelectedEmails.length} recipients in BCC`}
              >
                <Mail className="w-3.5 h-3.5" />
                <span>Send via My Gmail ({allSelectedEmails.length})</span>
              </button>
              ...
              <button
                ...
                onClick={handleSendBroadcast}
                disabled={sending}
                className="w-full sm:w-auto px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white font-black text-xs rounded-xl shadow-md flex items-center justify-center space-x-1.5 transition-all cursor-pointer"
              >
                <Send className="w-3.5 h-3.5" />
                <span>{sending ? 'Sending...' : `Dispatch in Portal (${allSelectedEmails.length})`}</span>
              </button>
